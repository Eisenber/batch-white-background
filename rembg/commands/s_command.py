...
from ..product_ui import WORKBENCH_CSS, create_product_ui
...
def s_command(port: int, host: str, log_level: str, threads: int, no_ui: bool) -> None:
    ...
    def gr_app(app):
        def product_session(model: str) -> BaseSession:
            cache_key = (model, "product-ui")
            session = sessions.get(cache_key)
            if session is None:
        ...
        workbench = create_product_ui(product_session)
        app = gr.mount_gradio_app(app, workbench, path="/", css=WORKBENCH_CSS)
        return app
